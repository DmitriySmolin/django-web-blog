...
from .forms import UserOurRegistration, UserUpdateForm, ProfileImg
...
def profile(request):
    if request.method == 'POST':
        img_profile = ProfileImg(request.POST, request.FILES, instance=request.user.profile)
        ...
        img_profile = ProfileImg(instance=request.user.profile)
